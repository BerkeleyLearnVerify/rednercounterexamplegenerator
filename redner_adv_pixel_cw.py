...
class SemanticPerturbations:
    def __init__(self, framework, filename, dims, label_names, normalize_params, background, pose,
        ...
        if attack_type == "CW":
            for _, mesh in mesh_list:
                ...
                modifier = torch.zeros(mesh.vertices.size(), requires_grad=False, device=pyredner.get_device())
            ...
            for _, mesh in mesh_list:
                vertices.append(mesh.vertices)
                mesh.vertices = Variable(mesh.vertices, requires_grad=False)
                mesh.vertices.retain_grad()
        ...
        self.translation = torch.tensor([0., 0., 0.], device=pyredner.get_device(), requires_grad=False)

        self.angle_input_adv_list = []
        self.angle_input_orig_list = []
        self.pose = pose
        if attack_type == "CW":
            self.euler_angles_modifier = torch.tensor([0., 0., 0.], device=pyredner.get_device(), requires_grad=False)
            if self.clamp_fn == "tanh":
                if pose == 'forward':
            ...
            if pose == 'forward':
                self.euler_angles = torch.tensor([0., 0., 0.], device=pyredner.get_device(), requires_grad=False)
            elif pose == 'top':
                self.euler_angles = torch.tensor([0.35, 0., 0.], device=pyredner.get_device(), requires_grad=False)
            elif pose == 'left':
                self.euler_angles = torch.tensor([0., 0.50, 0.], device=pyredner.get_device(), requires_grad=False)
            elif pose == 'right':
                self.euler_angles = torch.tensor([0., -0.50, 0.], device=pyredner.get_device(), requires_grad=False)
    ...
    def render_image(self, out_dir=None, filename=None):
        ...
        if self.attack_type == "CW":
            self.euler_angles_modifier.data = torch.tensor([0., 0., 0.], device=pyredner.get_device(), requires_grad=False)
            self.euler_angles = tanh_rescale(torch_arctanh(
                        torch.tensor([0., 0., 0.], device=pyredner.get_device())) + self.euler_angles_modifier)
        else:
    ...
    def attack_cw(self, label, out_dir=None, save_title=None, steps=5,
        ...
        self.orig_img = img
        self.img_modifier = torch.zeros(img.size(), device=pyredner.get_device(), requires_grad=True)
        ...
        for i in range(steps):
            ...
            loss = 0
            dist = l2_dist(self.orig_img, self.modified_img, False)
            ...
            if not torch.isfinite(self.img_modifier.grad).all():
                ...
                # subtract because we are trying to decrease the classification score of the label
                old_modifier = self.img_modifier.clone().detach()
                self.img_modifier.data -= (self.img_modifier.grad / (
                    torch.norm(self.img_modifier.grad) + delta)) * pixel_lr

                img = tanh_rescale(torch_arctanh(self.orig_img) - old_modifier + self.img_modifier)
