...
v = SemanticPerturbations(vgg16, obj_filename, dims=(224,224), label_names=label_names, 
                           normalize_params=vgg_params, background="blue_white.png", pose="left", num_classes=NUM_CLASSES, attack_type="FGSM")
if not os.path.exists('./demo_out'):
	os.mkdir('./demo_out')
pred, img = v.attack_FGSM(label=7, out_dir="demo_out/", save_title="motorcycle_demo_left.png", steps=5, vertex_eps=0.002, pose_eps=0.15, pose_attack=True, vertex_attack=True)