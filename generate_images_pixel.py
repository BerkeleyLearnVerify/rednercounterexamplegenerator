from redner_adv_pixel_cw import *
import argparse
import os
...
if attack_type is None:
    out_dir = "/work/yan.andy4/rednercounterexamplegenerator/out_pixel/benign/" + obj_id
else:
    out_dir = "/work/yan.andy4/rednercounterexamplegenerator/out_pixel/" + attack_type + "/" + args.params + "/" + obj_id
    ...
    for pose in poses:
        ...
        try:
            ...
            if attack_type is None:
                ...
                pred, img = v.attack_cw(label, out_dir=out_dir, save_title=hashcode + '_' + pose, steps=100, pixel_lr=3, target=target)
                plt.imsave(out_dir + "/" + hashcode + '_' + pose + ".png", np.clip(img[0].permute(1, 2, 0).data.cpu().numpy(), 0, 1))
